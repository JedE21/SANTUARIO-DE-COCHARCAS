...
import { submitMassRequest, type ActionResult } from '@/lib/actions';
...
export function MassRequestForm() {
  ...
  async function onSubmit(ev: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await submitMassRequest(formData);
    setResult(res);
    setLoading(false);
    ...
    <Card className="p-6">
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="block space-y-1">
            <span className="text-sm font-medium">Tipo de intención</span>
            <Select name="intention_type" defaultValue="Acción de gracias">
        ...
        <label className="block space-y-1">
          ...
          <Textarea name="intention" placeholder="Describe la intención de la misa" />
        </label>
        ...
        <label className="block space-y-1">
          ...
          <Textarea name="notes" placeholder="Información adicional para la coordinación pastoral" />
        </label>

        <Button type="submit" disabled={loading} className="w-full">
          {loading ? 'Enviando...' : 'Solicitar misa'}
        </Button>
        <p className="text-xs text-muted-foreground">
          Tus datos se usan únicamente para coordinar la solicitud pastoral.
        </p>
      </form>
    </Card>
  );
